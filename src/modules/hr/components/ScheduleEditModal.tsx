...
interface Props { schedule: WeeklyScheduleRow; onClose: () => void; onSuccess?: () => void; }

const DAYS_ORDER = [1,2,3,4,5,6,7];

function isVacationNotes(notes: string | null): boolean { return !!notes?.startsWith("VACACIONES"); }
...
export function ScheduleEditModal({ schedule, onClose, onSuccess }: Props) {
  ...
  const initDays = (): ScheduleDayInput[] => DAYS_ORDER.map(dow => {
    const ex = schedule.days.find(d => d.dayOfWeek === dow);
    return ex ?? { dayOfWeek: dow, isRestDay: true, morningStart: null, morningEnd: null, afternoonStart: null, afternoonEnd: null };
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await updateScheduleAction({
        scheduleId: schedule.id, employeeId: schedule.employeeId,
        effectiveFrom, effectiveTo: effectiveTo || null,
        notes: notes || null, isVacation, days,
      });
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <label className="flex items-center gap-2.5 text-sm cursor-pointer rounded-lg border border-dashed px-4 py-3 hover:bg-muted/30 transition-colors">
            ...
            <span>
              ...
              <span className="text-muted-foreground text-xs block">Todos los días quedan como descanso.</span>
            </span>
          </label>
                    ...
                    {days.map(d => (
                      <tr key={d.dayOfWeek} className={d.isRestDay ? "opacity-50" : ""}>
                        ...
                        <td className="px-3 py-2 text-center">
                          <input type="checkbox" checked={d.isRestDay} className="rounded"
                            onChange={e => updateDay(d.dayOfWeek, { isRestDay: e.target.checked,
                        ...
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-1">
                            <input type="time" value={d.morningStart ?? ""} disabled={d.isRestDay}
                            ...
                            <input type="time" value={d.morningEnd ?? ""} disabled={d.isRestDay}
                        ...
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-1">
                            <input type="time" value={d.afternoonStart ?? ""} disabled={d.isRestDay}
                              onChange={e => updateDay(d.dayOfWeek, { afternoonStart: e.target.value || null })}
                              className="w-24 rounded border border-input bg-background px-2 py-1 text-xs disabled:opacity-40" />
                            <span className="text-muted-foreground">–</span>
                            <input type="time" value={d.afternoonEnd ?? ""} disabled={d.isRestDay}
